...
import type { Artist, Genre, Movie, MovieArtist } from "@/generated/prisma/client";
import { getMovie } from "@/server-actions/movie/get";
...
type MovieWithRelations = Movie & {
    genres: Genre[];
    movieArtists: (MovieArtist & { artist: Artist })[];
  };
...
export default function MovieEditForm() {
  ...
  useEffect(() => {
    async function fetchData() {
        const [genresData, artistsData, movieData] = await Promise.all([
            getGenres(),
            getArtists(),
            getMovie(id),
          ]);
      ...
      setMovie(movieData as MovieWithRelations);
      setSelectedGenres(movieData?.genres.map((g) => g.id) || []);
      setMovieArtists(
        movieData?.movieArtists.map((ma) => ({
    ...
    <form action={handleSubmit} className="space-y-4 max-w-lg">
      ...
      <div>
        ...
        <Input
          ...
          step="0.01"
          required
          defaultValue={movie.price.toNumber()}
        />
      </div>
        ...
        {movieArtists.map((movieArtist, index) => (
          <div key={index} className="flex items-center gap-2 mt-2">
            ...
            <Select
              ...
              onValueChange={(value) => handleArtistChange(index, "role", value)}
            >
              <SelectTrigger>
            ...
            <Button type="button" onClick={() => removeArtist(index)} variant="destructive">
              Remove
            </Button>
          </div>
        ))}
